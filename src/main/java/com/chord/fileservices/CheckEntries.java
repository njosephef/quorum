...
public class CheckEntries implements Runnable
{
	...
	public void run()
	{
		...
		for (int i = 0; i < filenames.length; i++)
		{
			...
			{
				/*Hash every filename*/
				FileEntry entry = new FileEntry(filenames[i], node.getLocalID().getIP(), node.getLocalID().getPID(), node.getProperties().getFSPort());
		...
		Iterator<Map.Entry<IdKey, Set<FileEntry>>> it = null;
		try
		{
