...
public class Client
{
	...
	public static void main(String[] argv) throws IOException, InterruptedException
	{
	 ...
	 // Lame implementation - To be reviewed
        ...
        fileServicePort = r.nextInt(16383) + 49152;
        Thread fileServer = new Thread(new ServerThread(share, fileServicePort));
        ...
        while (true)
        {
            ...
            catch (NullPointerException nExc)
            {
                r.setSeed(System.currentTimeMillis());
                fileServicePort = r.nextInt(16383) + 49152;
                fileServer = new Thread(new ServerThread(share, fileServicePort));
            }
        }
...
//		FileServiceServer mainFrame = new FileServiceServer(thisNode);
		...
		/*Node creation*/
		myProperties = new NodeProperties(lanIP, fileServicePort, processID, thisNode.getLocalID().toString());
...
//			Log.addMessage("Successor List", Log.TABLES);
		...
		FileServiceServer mainFrame = new FileServiceServer(thisNode);
        System.out.println("thisNode.toString() " + thisNode.toString());
		mainFrame.fetchFile("A Brief History of Computing.pdf");
	}
}
