...
final class CheckPredecessorTask implements Runnable {
    ...
    public void run()
    {
        try
        {
            ...
            if (debug)
            {
                CheckPredecessorTask.logger.debug("Check predecessor method has been invoked.");
			}
            ...
            if (predecessor == null)
            {
                CheckPredecessorTask.logger.info("Nothing to check, as predecessor is null");
				return;
            }
            ...
            try
            {
                ...
                if (debug)
                {
                    CheckPredecessorTask.logger.debug("Predecessor reached!");
                }
            }
            catch (CommunicationException e)
            {
                if (debug)
                {
                    CheckPredecessorTask.logger.debug("Checking predecessor was NOT successful due "
            ...
            CheckPredecessorTask.logger.info("Invocation of check predecessor on node "
